...
author: "Report by Jonathan Cervas"
...
emails <- read.csv('/Users/cervas/Library/CloudStorage/GoogleDrive-jcervas@andrew.cmu.edu/My Drive/GitHub/amberson/amberson-emails.csv')
```

```{r, results='hide'}
registration <- read.table(file.path(directory, "voter_emails.txt"), header=F, sep=",")
...
```{r read-OpaVote}
# Read OpaVote Files
voted <- read.table(file.path(directory, "voted.txt"))
voted$voted <- 1

not_voted <- read.table(file.path(directory, 'not_voted.txt'))
not_voted$not_voted <- 1

# Exclude those who are disabled (not usual)
disabled_voted <- read.table(file.path(directory, 'disabled.txt'))
...
# Disabled
voted_notvoted_merge <- merge(voted_notvoted_merge,disabled_voted, by.x="Email", by.y="V1", all.x=T)

# Sort
units_notvoted <- sort(voted_merge$Unit[!voted_merge$voted %in% 1])

units_notvoted <- sort(voted_notvoted_merge$Unit[is.na(voted_notvoted_merge$disable) & is.na(voted_notvoted_merge$voted)])

# Visited OpaVote?
# visited <- read.table('/Users/cervas/Library/Mobile Documents/com~apple~CloudDocs/Downloads/visited.txt')
# visited$visited <- 1
# voted_notvoted_disabled_visited_merge <- merge(voted_notvoted_merge,visited, by.x="Email", by.y="V1", all.x=T)
...
# Reached Quorum?
```{r quorum}
paste0(round(votes_cast/votes_total, d=3) * 100, "%")
```
...
```{r}
paste0(100 * round(sum(voted$voted)/dim(registration)[1], d=3), "%")
```
